...
# if(g1 == "bacterial_infection" | g2 == "bacterial_infection"){
#         PVAL_THRESH = 0.05
#         LOG2FC_THRESH = 0
#         BASEMEAN_THRESH = 100
# }
...
## Get sig genes
all_sig_df <- readRDS(OCCURANCE_RDS)[[g1]][['all_sig_df']] %>% filter(grepl(g2,comp))
sig_genes_df <-  all_sig_df %>%                    
                        filter(padj < PVAL_THRESH) %>%                                             ## filter by p-values
                        filter(abs(log2FoldChange) > LOG2FC_THRESH) %>%                                 ## filter by log2FC
                        filter(baseMean > BASEMEAN_THRESH)                                               ## filter by baseMean
...
sig_genes_df <- sig_genes_df %>% 
                        # filter(n >= NUM_OVERLAP)                                          ## filter by occurances
